...
def render_pil(city_one_name, city_one_weather, out, city_two_name = None, city_two_weather = None):
    ...
    if city_two_weather:
        ...
        draw.text((400, 90), f"{city_two_weather.summary}", 'green', paragraph)
        draw.text((400, 120), f"{city_two_weather.weather}", 'purple', paragraph)
        draw.text((400, 150), f"Temp: {city_two_weather.temp}°F", 'black', paragraph)
